   ...
   ["make-char" "Make a local character using a GrimTools character"
    (str/join "\n"
              ["Syntax: make-char <character-url-or-id-or-file>"
               ...
               ""
               "Items the game may refuse at first"
               "---------------------------------"
               "A build reaches a heavy item's attribute requirement through the rest of"
               "its gear. That works in play, where the gear went on one piece at a time,"
               "but the game checks equipment when it loads a character it did not equip"
               "itself and cannot count a bonus from an item it has not accepted yet. So"
